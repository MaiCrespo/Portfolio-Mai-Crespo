  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        // Dramatic blur function with more samples
        vec4 blur(sampler2D tex, vec2 uv, float radius) {
          vec4 color = vec4(0.0);
          float total = 0.0;
          
          // More samples for stronger blur
          for(float x = -6.0; x <= 6.0; x += 1.0) {
            for(float y = -6.0; y <= 6.0; y += 1.0) {
              vec2 offset = vec2(x, y) * radius * 0.005;
        ...
        void main() {
          ...
          // Calculate distance from mouse with aspect ratio correction
          vec2 mouseUV = vec2(u_mouse.x, 1.0 - u_mouse.y);
          float aspectRatio = u_resolution.x / u_resolution.y;
          vec2 normalizedUV = vec2(uv.x * aspectRatio, uv.y);
          vec2 normalizedMouse = vec2(mouseUV.x * aspectRatio, mouseUV.y);
          float dist = distance(normalizedUV, normalizedMouse);
          
          // Create larger sharp area around mouse
          float sharpRadius = 0.3;
          float blurFalloff = 0.2;
          float blurAmount = smoothstep(sharpRadius, sharpRadius + blurFalloff, dist);
          
          // Sample sharp and heavily blurred versions
          vec4 sharpText = texture2D(u_textTexture, uv);
          vec4 blurredText = blur(u_textTexture, uv, u_blurRadius);
