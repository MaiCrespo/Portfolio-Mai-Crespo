  ...
  return (
    <div className="app-wrapper">
      <AnimatePresence mode="wait">
        {/* LANDING PAGE - Centered Logo with Shake */}
        {view === "landing" && (
          <motion.main
            key="landing"
        ...
        {/* SELECTION SCREEN - Logo on Right, Scattered Cards */}
        {view === "gallery" && (
          <motion.div
            key="gallery"
          ...
          >
            ...
            <div className="cards-flex-container">
              {/* Card 1: Tilted Left and Down */}
              <motion.img
                src={cardUI}
                className="tarot-card"
                initial={{ y: 200, opacity: 0, rotate: -15 }}
                animate={{ y: 40, opacity: 1, rotate: -8 }}
                whileHover={{ y: -20, rotate: 0, scale: 1.05, zIndex: 10 }}
              />
              {/* Card 2: Center, Tilted Slightly Right and Up */}
              <motion.img
                src={cardGraphic}
                className="tarot-card"
                initial={{ y: 200, opacity: 0, rotate: 5 }}
                animate={{ y: -20, opacity: 1, rotate: 3 }}
                whileHover={{ y: -50, rotate: 0, scale: 1.05, zIndex: 10 }}
                onClick={() => setView("graphicDesign")}
              />
              {/* Card 3: Tilted Far Right and Down */}
              <motion.img
                src={cardIllustration}
                className="tarot-card"
                initial={{ y: 200, opacity: 0, rotate: 15 }}
                animate={{ y: 60, opacity: 1, rotate: 12 }}
